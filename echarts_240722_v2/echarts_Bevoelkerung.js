...
if (Math.max(...data['Gesamt']) < 5000) {
    var axis_max_trend = Math.ceil(Math.max(...data['Entwicklung'])/50)*1000
    var axis_max_gesamt = Math.ceil(Math.max(...data['Gesamt'])/50)*60
}
else if (Math.max(...data['Gesamt']) < 20000) {
...
if (Math.min(...data['Entwicklung']) < 500) {
  var axis_min_trend = Math.floor(Math.min(...data['Entwicklung'])/200)*200
} else {
